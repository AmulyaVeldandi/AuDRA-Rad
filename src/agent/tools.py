...
class TaskGenerator:
    ...
    def build_order(self, state: AgentState, recommendation: Dict[str, Any]) -> ServiceRequest:
        ...
        patient_id = state.patient_id or (state.patient_context or {}).get("patient_id") or self.default_patient_id
...
def generate_task_tool(
    ...
    try:
        ...
        timeframe_months = recommendation.get("timeframe_months")
        days_in_future = 30
        if isinstance(timeframe_months, int) and timeframe_months > 0:
            days_in_future = max(timeframe_months * 30, 1)
        scheduled_date = date.today() + timedelta(days=days_in_future)

        task_payload = {
            "task_id": order_id,
            "order_id": order_id,
            "procedure": service_request.code.text or recommendation.get("follow_up_type"),
            "reason": recommendation.get("reasoning") or "Automated recommendation generated by AuDRA.",
            "scheduled_date": scheduled_date,
            "recommendation_id": recommendation.get("id"),
        }
        state.tasks_generated.append(task_payload)
